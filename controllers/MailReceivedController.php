...
class MailReceivedController extends Controller
{
    ...
    {    
        ...
        $dataProvider->query->andWhere(['status' => Mail::STATUS_RECEIVED]);
    ...
    public function actionCreate()
    {
        $request = Yii::$app->request;
        $model = new Mail();
        $model->status = Mail::STATUS_RECEIVED;
            ...
            if (!empty($model->fileUploading)) {
                ...
                foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
                    ...
                    for ($row = 2; $row <= $highestRow; ++ $row) {
                        ...
                        $newModel->status = Mail::STATUS_RECEIVED;
                        \Yii::warning($newModel->when_send, 'when_send');
                        $cell = $worksheet->getCellByColumnAndRow(6, $row);
                        $newModel->when_receive  = $cell->getValue();
