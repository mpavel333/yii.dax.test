...
class ClientController extends Controller
{
    ...
    {    
        ...
        $dataProvider->query->andWhere(['client.user_id' => Yii::$app->user->getId()]);
    ...
    public function actionPrint($id)
    {
        ...
        $templates = array_filter($templates, function($template){
            return mb_strpos($template->name, "Договор") !== false;
        });
    ...
    public function actionPrintFromClient($id)
    {
        ...
        $templates = \app\models\TemplateClient::find()
                ->where(['type' => Template::TYPE_FLIGHT])
                ->where(['id' => 4])
                ->all();
    ...
    public function actionPrintClientTemplate($id = null, $template_id = null, $signature = 0)
    {
        ...
        $template = \app\models\TemplateClient::findOne($template_id);
        $content = $template->text;
        $content = $client->tags($content);
        ...
        if($request->isAjax){
            ...
            */
            ...
            if($request->isGet){
                return [
                    ...
                    'content'=>$this->renderAjax('update-files', [
                        'model' => $model,
                    ]),
                    'footer'=> Html::button('Отмена',['class'=>'btn btn-default pull-left','data-dismiss'=>"modal"]).
                                Html::button('Сохранить',['class'=>'btn btn-primary','type'=>"submit"])
            ...
            */
            if ($model->load($request->post()) && $model->save()) {
                ...
                return $this->render('update-files', [
                    'model' => $model,
                ]);
            }
        }
    ...
    public function actionViewAjaxId($id)
    {
        Yii::$app->response->format = Response::FORMAT_JSON;
    ...
    public function actionGetDadata($inn)
    {
