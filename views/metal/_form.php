			...
			<div class="panel panel-default">
			    ...
			    <div class="panel-body">
			    	<div class="row">
				         ...
				         <div class="col-md-3">
							<div class="row">
								<?= $form->field($model, 'zakazchik_id', ['cols' => 6, 'colsOptionsStr' => " "])->widget(Select2::class, [
										'data' => $clientData,
										'options' => [
											'placeholder' => 'Выберите',
											'disabled' => $managerReadonlyDateTimeTop,
									...
									<?php
										$clientOrganizationData = \app\models\Client::find()->where(['type' => \app\models\Client::TYPE_CLIENT])->all();
										$clientMap = [];
										foreach ($clientOrganizationData as $client) {
											$clientMap[$client->id] = $client->name." ({$client->inn})";
										}
									
									?>
									<?= $form->field($model, 'client_id', ['cols' => 6, 'colsOptionsStr' => " "])->widget(Select2::class, [
										'data' => $clientMap,
										'options' => [
											'placeholder' => 'Выберите',
											'disabled' => $managerReadonlyDateTimeTop,
...
<?= $form->field($model, 'address_out2', ['cols' => 6, 'colsOptionsStr' => " "])->textInput(['readonly' => false])  ?>
        	...
        	<div class="row">
        		...
        		<div class="col-md-4">
        			<div class="row">
		        		...
		        		<?= $form->field($model, 'we_prepayment_form', ['cols' => 6, 'colsOptionsStr' => " "])->textInput(['readonly' => $managerReadonlyDateTime])->label('Аванс')  ?>
        			</div>
        		</div>
         ...
         <?php
             ...
             <div class="col-md-4">
             	<div class="row">
             		...
             		<?= $form->field($model, 'payment_out_prepayment_form', ['cols' => 6, 'colsOptionsStr' => " "])->textInput(['readonly' => $managerReadonlyDateTimeTop])->label('Аванс')  ?>
             	</div>
             </div>
         ...
         ?>
             ...
             <div class="col-md-4">
             	<div class="row">
             		<?= $form->field($model, 'client_payment', ['cols' => 6, 'colsOptionsStr' => " "])->textInput(['readonly' => $managerReadonlyDateTimeTop])->label('Оплата покупателя')  ?>
             		<?= $form->field($model, 'client_payment_prepayment', ['cols' => 6, 'colsOptionsStr' => " "])->textInput(['readonly' => $managerReadonlyDateTimeTop])->label('Аванс')  ?>
             	</div>
             </div>


                    
         <?= $form->field($model, 'client_payment_form', ['cols' => 3, 'colsOptionsStr' => " "])->dropDownList(app\models\Flight::otherwise2Labels(), ['prompt' => 'Выберите вариант', 'disabled' => $managerReadonlyDateTimeTop])->label('Форма оплаты') ?>
         ...
         // echo $form->field($model, 'otherwise3', ['cols' => 3, 'colsOptionsStr' => " "])->dropDownList(app\models\Flight::otherwise3Labels(), ['prompt' => 'Выберите вариант', 'disabled' => $managerReadonlyDateTime]) 
         	...
         	echo $form->field($model, 'client_payment_type', ['cols' => 3, 'colsOptionsStr' => " "])->widget(\kartik\select2\Select2::class, [
				'data' => ($model->otherwise3 && in_array($model->otherwise3, array_keys(app\models\Flight::otherwise3Labels())) == false) ? \yii\helpers\ArrayHelper::merge([$model->otherwise3 => $model->otherwise3], app\models\Flight::otherwise3Labels()) : app\models\Flight::otherwise3Labels(),
          		'options' => [
          			'placeholder' => 'Выберите вариант',
          			'disabled' => $managerReadonlyDateTimeTop,
         ...
         ?>
             ...
             <?= $form->field($model, 'client_payment_days', ['cols' => 2, 'colsOptionsStr' => " "])->textInput(['readonly' => $managerReadonlyDateTimeTop])->label('Кол-во дней')  ?>
             		...
             		<div class="panel-body" style="<?= $model->is_insurance == false ? "display: none;" : "" ?>">
             			<div class="row">
             				...
             				<?php
             					$ensuranceCompanies = [];
             					if($model->organization_id){
             						$ensuranceCompanies =\app\models\RequisiteEnsurance::find()->where(['requisite_id' => $model->organization_id])->all();
             					}
             				?>
...
foreach(\app\models\Client::find()->all() as $client)
{
             				...
             				<?php
						        ...
						        if($model->ensurance_contract_where){
							        ...
							        if($requisiteEnsurance){
							            $items = \app\models\RequisiteEnsurance::find()->where(['name' => $requisiteEnsurance->name])->all();
			...
			<?= $form->field($model, 'is_driver_signature', ['cols' => 2, 'colsOptionsStr' => " "])->checkbox()  ?>
             ...
             <div class="row">
               <div class="col-md-12">
                    ...
                    <div class="row">
        <div class="col-md-4" <?= \Yii::$app->user->identity->can('flight_upload_file') ? '' : 'style="display: none;"' ?>>
							<div class="row">
...
?>    
    ...
    'id'        => 'dzImage_file6',
    'uploadUrl' => \yii\helpers\Url::toRoute([ '/flight/upload-file']),
    'dropzoneContainer' => 'dzzzz-container-images',
    'previewsContainer' => 'preview-images-client', // <-- уникальные previewsContainer,
    'options' => [
    'dictDefaultMessage' => 'Перетащите файлы сюда',
    ],
    'clientEvents' => [
      'complete' => "function(file){
        var path = JSON.parse(file.xhr.response);
        if($('[name=\'file_file_client_path\']').val()){
          var value = JSON.parse($('[name=\'file_file_client_path\']').val());
        } else {
          var value = [];
        }        console.log(value, 'current value');
        value.push(path);
        console.log(value, 'new value');        $('[name=\'file_file_client_path\']').val(JSON.stringify(value));
        var arr = [ 'jpeg', 'jpg', 'gif', 'png' ];
        var strE = path.url;        
		//var ext = strE.substring(strE.lastIndexOf('.')+1);
        var ext = path.ext;
        ...
        html += '<td><p class=\"name\">'+path.name+'</p><p class=\"size\">'+path.size+'</p></td>';
        html += '<td><a data-delete=\"'+path.url+'\" class=\"btn btn-danger cancel\"><i class=\"glyphicon glyphicon-ban-circle\"></i><span>Удалить</span></a></td></tr>';
        $('.files-client').append(html);
        $('.files-client [data-delete]').unbind('click');
        $('.files-client [data-delete]').click(deleteEventHandler);
        $('.files-client [data-delete]').on('keydown', keyDownEventHandler);
      }",
     ],
]);?>
<div style="display: none;">
  <input type="hidden" name="file_file_client_path">
</div>
<?php
$script = <<< JS
...
var deleteEventHandler = function(e){
  ...
  if($('[name=\'file_file_client_path\']').val()){
    var data = JSON.parse($('[name=\'file_file_client_path\']').val());
  } else {
    var data = [];
  }
  ...
  $('[name=\'file_file_client_path\']').val(JSON.stringify(newData));
...
?>
                ...
                <div class="col-md-7" style="height: 200px; overflow: auto;">
                    <table role="presentation" class="table table-striped">
                        <tbody class="files-client">
                        <?php if ($model->file_client):
								$counter = 0; 
								$files = json_decode($model->file_client, true);
								if($files):
								foreach ($files as $value):?> 
							<?php if(isset($value['url']) == false) { continue; } ?>
                          <tr class="template-upload fade in">
...
?>    
                    	...
                    	<div class="col-md-4" <?= \Yii::$app->user->identity->can('flight_driver_upload_file') ? '' : 'style="display: none;"' ?>>
                    		<div class="row">
                <div class="col-md-5" style="height: 200px; overflow: auto;">
