<?php foreach($messages as $message): ?>
    ...
    <div class="panel panel-<?= $panelClass ?>">
        ...
        <div class="panel-body">
            ...
            <?php
                $application = json_decode($message->application, true);
            ?>
            <?php if($application > 0): ?>
                <p class="message-header">Файлы:</p>
                <ul class="message-list">
                    <?php foreach($application as $file): ?>
                        <li><a href="/<?= $file['path'] ?>" download="<?= $file['name'] ?>"><?= $file['name'] ?></a></li>
                    <?php endforeach; ?>
                </ul>
            <?php endif; ?>
        </div>
