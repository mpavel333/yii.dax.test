          ...
          <div style="display: none;">
            ...
            <div class="col-md-4">
              <div class="row">
                ...
                <?php //= $form->field($model, 'dadata_inn', ['cols' => 12, 'colsOptionsStr' => " "])->textInput()  ?>
            ...
            <div class="col-md-4">
              <div class="row">
                ...
                <?= $form->field($model, 'comment', ['cols' => 12, 'colsOptionsStr' => " "])->textarea()  ?>
                  
                <?= $form->field($model, 'tabel', ['cols' => 12, 'colsOptionsStr' => " "])->textInput()  ?>
              </div>
            </div>
            <div class="col-md-4">
              <div class="row">
                ...
                <?php 
                    $disabled = false;
                    if($model->contract == 1 or $model->contract_orig == 1) 
                        $disabled = true;
                ?>  
                  
                <?= $form->field($model, 'doc', ['cols' => 6, 'colsOptionsStr' => " "])->textInput(['readonly' => $disabled])  ?>

                <?= $form->field($model, 'doc_date', ['cols' => 6, 'colsOptionsStr' => " "])->textInput(['type' => 'date','readonly' => $disabled])  ?>
                ...
                if(\Yii::$app->user->identity->can('client_contract')): ?>
                    <?= $form->field($model, 'contract',['cols' => 3, 'colsOptionsStr' => " "])->checkbox()  ?>
                <?php else: ?>
                  <?= $form->field($model, 'contract',['cols' => 3, 'colsOptionsStr' => " "])->checkbox(['disabled' => true])  ?>
                <?php endif; ?>   
                
                <?php if(\Yii::$app->user->identity->can('client_contract')): ?>
                    <?= $form->field($model, 'contract_orig',['cols' => 3, 'colsOptionsStr' => " "])->checkbox()  ?>
                <?php else: ?>
                  <?= $form->field($model, 'contract_orig',['cols' => 3, 'colsOptionsStr' => " "])->checkbox(['disabled' => true])  ?>
                <?php endif; ?>       
                  
            
                
                <?php if(\Yii::$app->user->identity->can('client_contract')): ?>
                    <?= $form->field($model, 'payment_terms',['cols' => 3, 'colsOptionsStr' => " "])->textInput()  ?>
                <?php else: ?>
                  <?= $form->field($model, 'payment_terms',['cols' => 3, 'colsOptionsStr' => " "])->textInput(['disabled' => true])  ?>
                <?php endif; ?>    
                  
                <?php if(\Yii::$app->user->identity->can('client_contract')): ?>
                    <?= $form->field($model, 'payment_type',['cols' => 3, 'colsOptionsStr' => " "])->textInput()  ?>
                <?php else: ?>
                  <?= $form->field($model, 'payment_type',['cols' => 3, 'colsOptionsStr' => " "])->textInput(['disabled' => true])  ?>
                <?php endif; ?>   
                ...
                if($model->limit == null){
                  ...
                  if($salary && $salary->limit)
                  {
                    $model->limit = $salary->limit;
                  }
                }

                ?>
                <?php if(\Yii::$app->user->identity->can('client_limit_visible')): ?>
                    <?= $form->field($model, 'limit',['cols' => 12, 'colsOptionsStr' => " "])->textInput()  ?>
                <?php else: ?>
                  <?= $form->field($model, 'limit',['cols' => 12, 'colsOptionsStr' => " "])->textInput(['readonly' => true])  ?>
                <?php endif; ?>
...
$("#client-inn").change(function(){
  ...
  $.get("/client/view-ajax?inn="+value, function(response){
    ...
    if(response.model != null){
      ...
      $('#client-kr').val(response.model.kr);
      $('#client-name_case').val(response.model.name_case);
    } else {
      $('#idHiddenInput').val(null);

          $.get("/client/get-dadata?inn="+value, function(response){
              console.log(response);

              if(response.model != null){
                $('#client-name').val(response.model.value);
                $('#client-doljnost_rukovoditelya').val(response.model.data.management.post);
                $('#client-fio_polnostyu').val(response.model.data.management.name);
                $('#client-official_address').val(response.model.data.address.value);

                //$('#client-bank_name').val(response.model.bank_name);
                $('#client-kpp').val(response.model.data.kpp);
                $('#client-ogrn').val(response.model.data.ogrn);
                //$('#client-bic').val(response.model.bic);
