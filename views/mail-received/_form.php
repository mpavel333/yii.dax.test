...
foreach (\app\models\Client::find()->all() as $item) {
  $clientData[$item->id] = "{$item->name} ({$item->mailing_address})";
}
...
<div class="mail-form">
    ...
    <div class="row">
             ...
             <?= $form->field($model, 'client_id', ['cols' => 12, 'colsOptionsStr' => " "])->widget(\kartik\select2\Select2::class, [
                'data' => $clientData,
            ])  ?>
    
                
             <?= $form->field($model, 'information', ['cols' => 12, 'colsOptionsStr' => " "])->textInput()->label('Информация письма')  ?>
...
$("#mail-upd").change(function(){
  $.get("/flight/get-by-attr?attr=upd&value="+$(this).val(), function(response){ 
    $("#mail-track").val(response.track_number); 
	});
});
