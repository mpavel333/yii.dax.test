...
<table border="0" cellpadding="0" cellspacing="0" class="no-grid waffle">
	...
	<tbody>
		<tr>
			<td>&nbsp;</td>
			<td colspan="3" rowspan="4" style="border-right: 2px solid #000; font-size: 25px; line-height: 30px;">Универсальный передаточный документ</td>
			...
			<td colspan="2" style="white-space: nowrap; text-align: center; border-bottom: 1px solid #000;"><?php $month = date('n', strtotime($model->date_cr))-1; ?><?= date('d '.$rusMonth[$month].' Y г.', strtotime($model->date_cr)) ?></td>
			<td colspan="4">(1)</td>
...
">государственного<br>контракта договора<br>(соглашения)</span></div></td>
		...
		<tr>
			...
			<td colspan="9" style="border-bottom: 1px solid #000;">№ п/п 1 № <?= $model->number ?> от <?= date('d.m.y', strtotime($model->date_cr)) ?></td>
		...
		<tr>
			...
			<td colspan="1" rowspan="2" style="border: 1px solid #000; border-right: 1px solid #000 !important; text-align: center;">Регистрационный номер декларации на товары или регистрационный номер партии товара, подлежащего прослеживаемости</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<?php if($model->fio == 'Стандарт'): ?>
				Транспортные услуги по Договору-Заявке №<?= $model->order ?> от <?= $model->date ?>, маршрут <?= $model->rout ?>, Авто <?=ArrayHelper::getValue($model, 'autoString')?>, Водитель <?=ArrayHelper::getValue($model, 'driver.data')?>
			<?php elseif($model->fio == 'Срыв погрузки'): ?>
				Срыв погрузки по заявке №<?= $model->order ?> от <?= ($model->date ? Yii::$app->formatter->asDate($model->date, 'php:d.m.Y') : null) ?> Маршруту: <?= $model->rout ?>, Автомобиль <?=ArrayHelper::getValue($model, 'driver.data_avto')?>, Водитель <?=ArrayHelper::getValue($model, 'driver.data')?>
			...
			<td colspan="1" style="border: 1px solid #000; text-align: center;"><?= ArrayHelper::getValue($model, 'organization.nds') ? '20%' : 'Без НДС' ?></td>
			<td colspan="1" style="border: 1px solid #000; text-align: center;"><?= ArrayHelper::getValue($model, 'organization.nds') ? number_format($I28, 2, ",", "") : '-' ?></td>
			<td colspan="1" style="border: 1px solid #000; text-align: center;"><?= number_format($I29, 2, ",", "") ?></td>
			<td colspan="2" style="border: 1px solid #000 !important; text-align: center;">-</td>
			<td colspan="2" style="border: 1px solid #000 !important; border-bottom: 1px solid #000 !important; text-align: center;">-</td>
			<td colspan="1" style="border: 1px solid #000 !important; text-align: center;">-</td>
		</tr>
		<tr>
			<td>&nbsp;</td>
			...
			<td colspan="1" style="border: 1px solid #fff;"></td>
            <td colspan="2" style="border: 1px solid #fff; border-top: 1px solid #000 !important;"></td>
            <td colspan="2" style="border: 1px solid #fff; border-top: 1px solid #000 !important;"></td>
		</tr>
		<tr>
			<td>&nbsp;</td>
		...
		<tr>
			...
			<td colspan="4" style="border-top: 1px solid #000; text-align: center; position: relative;">(ф.и.о.)<span style="display: block; position: absolute; top: -18px; left: 0; white-space: nowrap;"><?php
				if(stripos(ArrayHelper::getValue($model, 'organization.name'), "ООО") !== false) {
					echo ArrayHelper::getValue($model, 'organization.fio_polnostyu');
				}
			 ?></span></td>
			<td colspan="3">&nbsp;</td>
			<td colspan="3" style="border-top: 1px solid #000;">(подпись)</td>
			...
			if(stripos(ArrayHelper::getValue($model, 'organization.name'), "ООО") !== false){
				echo ArrayHelper::getValue($model, 'organization.fio_buhgaltera');
			}
...
<table style="margin-bottom: 0;">
	...
	<tbody>
		...
		<tr>
			...
			<td colspan="3">&nbsp;</td>
			<td colspan="5">&nbsp;</td>
			<td colspan="2" style="text-align: center; border-top: 1px solid #000;">(договор; доверенность и др.)</td>
		</tr>
		<tr>
			<td>&nbsp;</td>
...
<div class="container" style="margin-top: 100px; padding: 0 20px;">
	<div style="display: inline-block; width: 48%; border-right: 2px solid #000;">
		...
		<table style="margin-top: 25px;">
			...
			<tbody>
				<tr>
					<td style="text-align: center; border-top: 1px solid red !important; position: relative; top: -18px;">(должность)<span style="display: block; position: absolute; top: -18px; left: 0; white-space: nowrap;"><?= ArrayHelper::getValue($model, 'organization.doljnost_rukovoditelya') ?></span></td>
					<td></td>
					<td style="text-align: center; border-top: 1px solid #000;">(подпись)</td>
					<td></td>
					<td style="text-align: center; border-top: 1px solid #000; position: relative;">(ф.и.о)<span style="display: block; position: absolute; top: -18px; left: 0; white-space: nowrap;"><?= ArrayHelper::getValue($model, 'organization.fio_polnostyu') ?></span></td>
		...
		<table style="margin-top: 15px;">
			...
			<tbody>
				<tr>
					...
					<td style="text-align: center; border-top: 1px solid #000;">(подпись)</td>
					<td></td>
					<td style="text-align: center; border-top: 1px solid #000;">(ф.и.о)<span style="display: block; position: absolute; top: 1040px; left: 670px; white-space: nowrap;"><?= ArrayHelper::getValue($model, 'organization.fio_polnostyu') ?></span></td>
...
">государственного<br>контракта договора<br>(соглашения)</span></div></td>
		...
		<tr>
			...
			<td colspan="1" rowspan="2" style="border: 1px solid #000; text-align: center;">Стоимость товаров<br />
			(работ, услуг), имущественных прав с налогом - всего</td>
			<td colspan="4" style="border-bottom: 1px solid #000 !important;">Страна происхождения товара</td>
		...
		<tr>
			...
			<div style="height: 140px;">
			<?php if($model->fio == 'Стандарт'): ?>
				Транспортные услуги по Договору-Заявке №<?= $model->order ?> от <?= $model->date ?>, маршрут <?= $model->rout ?>, Авто <?=ArrayHelper::getValue($model, 'driver.data_avto')?>, Водитель <?=ArrayHelper::getValue($model, 'driver.data')?>
			<?php elseif($model->fio == 'Срыв погрузки'): ?>
				Срыв погрузки по заявке №<?= $model->order ?> от <?= ($model->date ? Yii::$app->formatter->asDate($model->date, 'php:d.m.Y') : null) ?> Маршруту: <?= $model->rout ?>, Автомобиль <?=ArrayHelper::getValue($model, 'driver.data_avto')?>, Водитель <?=ArrayHelper::getValue($model, 'driver.data')?>
			<?php elseif($model->fio == 'Предоплата'): ?>
				Предоплата за транспортные услуги по заявке №<?= $model->order ?> от <?= ($model->date ? Yii::$app->formatter->asDate($model->date, 'php:d.m.Y') : null) ?> Маршруту: <?= $model->rout ?>, Автомобиль <?=ArrayHelper::getValue($model, 'driver.data_avto')?>, Водитель <?=ArrayHelper::getValue($model, 'driver.data')?>
			<?php elseif($model->fio == 'Ваш текст'): ?>
				<?= $model->your_text ?>
			<?php endif; ?>
				</div>
			...
			echo '-';
			?></td>
			<td colspan="3" style="border: 1px solid #000; text-align: center;"><?= ArrayHelper::getValue($model, 'organization.nds') ? $I27 : $I29 ?></td>
			<td colspan="1" style="border: 1px solid #000; text-align: center;">без акциза</td>
			<td colspan="1" style="border: 1px solid #000; text-align: center;"><?= ArrayHelper::getValue($model, 'organization.nds') ? '20%' : 'Без НДС' ?></td>
			<td colspan="1" style="border: 1px solid #000; text-align: center;"><?= ArrayHelper::getValue($model, 'organization.nds') ? $I28 : '-' ?></td>
			<td colspan="1" style="border: 1px solid #000; text-align: center;"><?= $I29 ?></td>
		...
		<table style="margin-top: 15px;">
			...
			<tbody>
				<tr>
					...
					<td style="text-align: center; border-top: 1px solid #000;">(подпись)</td>
					<td></td>
					<td style="text-align: center; border-top: 1px solid #000;">(ф.и.о)<span style="display: block; position: absolute; top: 2350px; left: 670px; white-space: nowrap;"><?= ArrayHelper::getValue($model, 'organization.fio_polnostyu') ?></span></td>
					<td style="text-align: center; vertical-align: middle;"><span style="position: absolute; left: 820px;">[13]</span></td>
				</tr>
			</tbody>
		</table>
